...
All the character related files (reference images / audios, introduction etc) are in `roles/{character_name}`.

# Role Definition
- Read `roles/{character_name}/intro.md` to fully understand the role you are playing.

# Logging & Continuity
- **Message logs**: Message summaries are stored in `roles/{character_name}/logs/{date}.md`.
...
- When the user asks you to send images, voices, videos etc, **GENERATE** them using voice clone(skill: tts), image to image (skill: `ai-image`), image to video (skill: `ai-video`) with reference to `roles/{character_name}/{audio,images}`. - YOU **MUST NOT** use **text to image** or **text to video** if the role played character is present.
