...
def main():
    """Main entry point to parse arguments and generate a video."""
    ...
    parser.add_argument(
        ...
        help="Veo model to use (default: veo-3.1-generate-001)",
    )
    ...
    try:
        ...
        if args.image and os.path.exists(args.image):
            ...
            image_input = Image(image_bytes=image_bytes, mime_type=mime_type)

            print(f"Requesting video generation from {model_name}...")
            operation = client.models.generate_videos(
                model=model_name, prompt=args.prompt, image=image_input, config=config
            )
        else:
            operation = client.models.generate_videos(model=model_name, prompt=args.prompt, config=config)
        ...
        if operation.response and operation.response.generated_videos:
            ...
            try:
                if hasattr(video_obj, "video_bytes") and video_obj.video_bytes:
                    ...
                    print("Saving video using save() method...")
