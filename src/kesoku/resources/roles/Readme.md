...
## 🧠 Memory Isolation (`memo` category)

To ensure that different characters do not confuse their interactions:
- Globally shared knowledge categories (`progress`) are always stored in the globally shared `"default"` role scope.
- The `"memo"` memory category is role-isolated. Whenever the agent is active under a specific persona, facts and memorable daily events are stored privately under that character's role scope (e.g., `role='asuka'`).
