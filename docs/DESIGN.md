...
- **Stop Turn button & Turn Metrics**: The bottom of the active foldable UI card features a `Stop Turn 🛑` button during the turn's active execution. Once the turn finishes successfully or gets stopped/interrupted by the user, the button disappears and is replaced by detailed turn and session metrics (total session turns, context window size, executed tool calls, turn token usage, and elapsed time), signaling the final state of the turn clearly.
- **Interactive Choice Question Card**: Multiple-choice question blocks (`[question: ... | choices]`) are parsed and presented visually as vertical interactive button cards, posting selections back to Gateway on behalf of the clicking user.
